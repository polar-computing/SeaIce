...
def trainingSamplesMouse(img,segments,maxClasses=3,iclasses=None):
    ...
    """
	helpText = "'Left Click' to select segements \n'Right Click' to change class"
	print helpText
	...
	#Copy originial image into global
	bounded = img.copy()
...
# def plot(img,classified_image,iclasses):
# 	tmp = np.multiply((256.0/len(iclasses)),classified_image)
# 	tmp = np.column_stack((tmp,img))
# 	plt.imshow(tmp,cmap='gist_rainbow')
# 	plt.show()
#
